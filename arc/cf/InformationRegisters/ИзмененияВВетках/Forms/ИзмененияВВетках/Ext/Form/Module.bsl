...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияСППРКлиентСервер.НеобходимОтборПоКолонкеПриСозданииНаСервере("СтатусВетки", СписокСтатусовВеток, СтруктураБыстрогоОтбора) Тогда
		УстановитьОтборПоСтатусуВетки();
	КонецЕсли;
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если ОбщегоНазначенияСППРКлиентСервер.НеобходимОтборПоКолонкеПередЗагрузкойИзНастроек("СтатусВетки",
			"СписокСтатусовВеток",
			СписокСтатусовВеток,
			СтруктураБыстрогоОтбора,
			Настройки) Тогда
...
&НаКлиенте
Процедура СписокСтатусовВетокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	СтруктураПараметров = Новый Структура("СписокВыбора", СписокСтатусовВеток);
	
	ОткрытьФорму("Перечисление.СтатусыВеток.Форма.ВыборСпискаСтатусов", СтруктураПараметров, Элемент);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСтатусовВетокОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	СписокСтатусовВеток = ВыбранноеЗначение;
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСтатусовВетокПриИзменении(Элемент)
...
Процедура ПерейтиККоммитамВетки(Команда)
	...
	ТекстКоманды = ПерейтиККоммитамВеткиСервер(Элементы.Список.ТекущаяСтрока);
...
Процедура ПерейтиККоммитамПоМетаданному(Команда)
	...
	ИзмененыеФайлы = ИзмененыеФайлыДляПереходаККоммитамПоМетаданному(Элементы.Список.ТекущаяСтрока,
		"commits");
...
Процедура ПерейтиКСравнениюСВеткойПриемником(Команда)
	...
	ИзмененыеФайлы = ПерейтиКСравнениюСВеткойПриемникомСервер(Элементы.Список.ТекущаяСтрока);
...
Процедура ИзмененияСПредыдущегоСогласованияПоВсемОбъектам(Команда)
	...
	ТекстКоманды = ГиперссылкаИзмененияСПредыдущегоСогласованияПоВсемОбъектам(Элементы.Список.ТекущаяСтрока);
	Если НЕ ЗначениеЗаполнено(ТекстКоманды) Тогда
		Возврат;
	КонецЕсли;	
...
Процедура ИзмененияСПредыдущегоСогласованияПоТекущемуОбъекту(Команда)
	...
	ИзмененыеФайлы = ИзмененыеФайлыДляПереходаКИзменениямСПоследнегоСогласования(Элементы.Список.ТекущаяСтрока);
...
Процедура СравнитьСВеткойПриемникомНаКоммитСогласования(Команда)
	...
	ИзмененыеФайлы = ИзмененыеФайлыДляПереходаКСравнениюВеткиПриемникаСПредыдущимКоммитомСогласования(Элементы.Список.ТекущаяСтрока);
...
&НаСервере
Процедура УстановитьОтборПоСтатусуВетки()
...
Процедура УстановитьОтборПоПроекту()
	...
	Если НЕ Параметры.Отбор.Свойство("ОбъектМетаданных")
		И НЕ Параметры.Отбор.Свойство("ПодчиненныйОбъект") Тогда
...
Процедура УстановитьДоступностьЭлементов()
	...
	Если Элементы.Найти("СписокКонтекстноеМенюНазначитьОтветственногоИСогласующего") <> Неопределено Тогда
		Элементы.СписокКонтекстноеМенюНазначитьОтветственногоИСогласующего.Доступность =
			ДоступноИзменениеОбъектовМетаданных;
	КонецЕсли;
...
Функция ПерейтиККоммитамВеткиСервер(КлючЗаписи)
	
	ДанныеИзЗаписиРС = ДанныеИзЗаписиРС(КлючЗаписи);
	
	ВеткаОтбор = ДанныеИзЗаписиРС.Ветка;
	
	Если НЕ ЗначениеЗаполнено(ВеткаОтбор) Тогда
		Возврат Неопределено;
	КонецЕсли;  
	
	Возврат РаботаCGit.ПерейтиККоммитамВетки(ВеткаОтбор);
	
КонецФункции

&НаСервереБезКонтекста
Функция ИзмененыеФайлыДляПереходаККоммитамПоМетаданному(КлючЗаписи,  Тип)
	ДанныеИзЗаписиРС = ДанныеИзЗаписиРС(КлючЗаписи);
	...
	Возврат РаботаCGit.ИзмененыеФайлыДляПереходаККоммитамПоМетаданному(
		ДанныеИзЗаписиРС.ПодробноеОписаниеИзменений, ВеткаОтбор, Тип);
	
КонецФункции

&НаСервереБезКонтекста
Функция ИзмененыеФайлыДляПереходаКСравнениюВеткиПриемникаСПредыдущимКоммитомСогласования(КлючЗаписи)
	ДанныеИзЗаписиРС = ДанныеИзЗаписиРС(КлючЗаписи);
	...
	Возврат РаботаCGit.ИзмененыеФайлыДляПереходаКСравнениюВеткиПриемникаСПредыдущимКоммитомСогласования(
		ДанныеИзЗаписиРС.ПодробноеОписаниеИзменений, ВеткаОтбор, ДанныеИзЗаписиРС.ОбъектМетаданных, ДанныеИзЗаписиРС.ПодчиненныйОбъект);
...
&НаСервереБезКонтекста
Функция ГиперссылкаИзмененияСПредыдущегоСогласованияПоВсемОбъектам(КлючЗаписи)
	...
	Возврат РаботаCGit.ГиперссылкаИзмененияСПредыдущегоСогласованияПоВсемОбъектам(
		ВеткаОтбор, ДанныеИзЗаписиРС.ОбъектМетаданных, ДанныеИзЗаписиРС.ПодчиненныйОбъект);
	
КонецФункции   

&НаСервереБезКонтекста
Функция ИзмененыеФайлыДляПереходаКИзменениямСПоследнегоСогласования(КлючЗаписи)
	...
	Возврат РаботаCGit.ИзмененыеФайлыДляПереходаКИзменениямСПоследнегоСогласования(
		ДанныеИзЗаписиРС.ПодробноеОписаниеИзменений, ВеткаОтбор, ДанныеИзЗаписиРС.ОбъектМетаданных, ДанныеИзЗаписиРС.ПодчиненныйОбъект);
	
КонецФункции

&НаСервереБезКонтекста
Функция ПерейтиКСравнениюСВеткойПриемникомСервер(КлючЗаписи)
	...
	Возврат РаботаCGit.ПерейтиКСравнениюСВеткойПриемникомСервер(ВеткаОтбор, ПодробноеОписаниеИзменений);
...
Процедура ПоказатьСравнениеФайлаССостояниемВеткиПриемника(ИмяФайла)
	...
	ТекстыМодулейДляСравнения = ТекстыМодулейДляСравненияССостояниемВеткиПриемника(Элементы.Список.ТекущаяСтрока, ИмяФайла);
	
	ПараметрыСравнения = Новый Структура;
	ПараметрыСравнения.Вставить("ТекстДляСравнения2", ТекстыМодулейДляСравнения.ТекущийТекстФайла);
	ПараметрыСравнения.Вставить("ОписаниеТекста2", НСтр("ru = 'Текущее значение в ветке'"));
	ПараметрыСравнения.Вставить("ТекстДляСравнения1", ТекстыМодулейДляСравнения.ТекстФайлаВВеткеПриемнике);
...
Процедура ПоказатьСравнениеФайлаССостояниемКоммитаСогласования(ДанныеФайла)
	...
	ТекстыМодулейДляСравнения = ТекстыМодулейДляСравненияССостояниемКоммитаСогласования(Элементы.Список.ТекущаяСтрока, ДанныеФайла);
	
	ПараметрыСравнения = Новый Структура;
	ПараметрыСравнения.Вставить("ТекстДляСравнения2", ТекстыМодулейДляСравнения.ТекущийТекстФайла);
	ПараметрыСравнения.Вставить("ОписаниеТекста2", НСтр("ru = 'Текущее значение в ветке'"));
	ПараметрыСравнения.Вставить("ТекстДляСравнения1", ТекстыМодулейДляСравнения.ТекстФайлаНаКоммит);
	ПараметрыСравнения.Вставить("ОписаниеТекста1", НСтр("ru = 'Значение на момент согласования'"));
	ПараметрыСравнения.Вставить("ИмяФайлаРепозитория", ДанныеФайла.ИмяФайла);
	ТестированиеКлиент.ПоказатьОкноСравнениеТекста(ПараметрыСравнения);
КонецПроцедуры
...
Функция ДанныеИзЗаписиРС(Знач КлючЗаписи)
	...
	Результат.Вставить("Ветка", Запись.Ветка);
...
&НаСервереБезКонтекста
Функция ТекстыМодулейДляСравненияССостояниемВеткиПриемника(КлючЗаписи, ИмяФайла)
	ДанныеИзЗаписиРС = ДанныеИзЗаписиРС(КлючЗаписи);
	ВеткаОтбор = ДанныеИзЗаписиРС.Ветка;
	...
	Результат = Новый Структура;
	Результат.Вставить("ТекущийТекстФайла", ТекущийТекстФайла);
	Результат.Вставить("ТекстФайлаВВеткеПриемнике", ТекстФайлаВВеткеПриемнике);
...
&НаСервереБезКонтекста
Функция ТекстыМодулейДляСравненияССостояниемКоммитаСогласования(КлючЗаписи, ДанныеФайла)
	ДанныеИзЗаписиРС = ДанныеИзЗаписиРС(КлючЗаписи);
	ВеткаОтбор = ДанныеИзЗаписиРС.Ветка;
	...
	Результат = Новый Структура;
	Результат.Вставить("ТекущийТекстФайла", ТекущийТекстФайла);
	Результат.Вставить("ТекстФайлаНаКоммит", ТекстФайлаНаКоммит);
