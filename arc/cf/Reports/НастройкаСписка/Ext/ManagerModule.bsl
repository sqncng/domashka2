...
Процедура ПрименитьНастройкуКСписку(ДанныеСписка, АдресСхемыКомпоновкиДанных, НастройкиКомпоновки, ПорядокПоУмолчанию = "") Экспорт
	...
	Если ТипЗнч(ДанныеСписка) = Тип("ТаблицаЗначений") Тогда
		НаборДанных = ДанныеСписка.СкопироватьКолонки();
		ЭтоТаблицаЗначений = Истина;
	Иначе
		НаборДанных = ДанныеСписка.Выгрузить().СкопироватьКолонки();
		ЭтоТаблицаЗначений = Ложь;
	КонецЕсли;
	...
	Для каждого ДанныеСтроки Из ДанныеСписка Цикл
		...
		НоваяСтрока._Идентификатор = ?(ЭтоТаблицаЗначений, ДанныеСтроки.Идентификатор, ДанныеСтроки.ПолучитьИдентификатор());
	КонецЦикла;
	...
	Для каждого ДанныеСтроки Из ДанныеСписка Цикл
		ДанныеСтроки.НеСоответствуетНастройкеСписка = (ТаблицаРезультата.Найти(?(ЭтоТаблицаЗначений, ДанныеСтроки.Идентификатор, ДанныеСтроки.ПолучитьИдентификатор()), "_Идентификатор") = Неопределено);
	КонецЦикла;
...
Функция ИнициализацияНастройки(РеквизитыСписка, НастройкиСписка, УникальныйИдентификаторФормы, НастройкаПолей = Неопределено) Экспорт
	...
	ИспользоватьНастройкуОтбора = Истина;
	ИспользоватьНастройкуСортировки = Истина;
	...
	Если НастройкаПолей <> Неопределено Тогда
		...
		Если НастройкаПолей.Свойство("ИспользоватьНастройкуОтбора") Тогда
			ИспользоватьНастройкуОтбора = НастройкаПолей.ИспользоватьНастройкуОтбора;
		КонецЕсли;
		Если НастройкаПолей.Свойство("ИспользоватьНастройкуСортировки") Тогда
			ИспользоватьНастройкуСортировки = НастройкаПолей.ИспользоватьНастройкуСортировки;
		КонецЕсли;
	КонецЕсли;
	...
	Если ИспользоватьНастройкуОтбора Тогда
	...
	КонецЕсли;
	Если ИспользоватьНастройкуСортировки Тогда
