...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТехническиеПроекты" Тогда
		...
		Если ВыбраннаяСтрока<>Неопределено Тогда
			...
			Если ДанныеСтроки.ТехническиеПроекты<>0 Тогда
				ПараметрыФормы = Новый Структура("Идея", ДанныеСтроки.Ссылка);
				ОткрытьФорму("Справочник.ТехническиеПроекты.Форма.СписокТехническихПроектовИспользующихИдею",
				ПараметрыФормы,
				ЭтаФорма);
			КонецЕсли;
...
Процедура ПеренестиИдеиВРаздел(МассивИдей, ИдентификаторСтрокиДерева)
	...
	Для Каждого Идея из МассивИдей Цикл
		...
		Исключение
			Сообщение = НСтр(" ru= ""Не удалось отнести идею %Идея% к тематике: %Тематика%
...
Процедура ОтнестиИдеиКТематике(МассивИдей, Тематика)
	...
	Для Каждого Идея из МассивИдей Цикл
		...
		Попытка
			Объект.Заблокировать();
		Исключение
			Сообщение = НСтр(" ru= ""Не удалось отнести идею %Идея% к тематике: %Тематика% по причине: %ОписаниеОшибки%""; ");
			Сообщение = СтрЗаменить(Сообщение, "%Идея%", Идея);
