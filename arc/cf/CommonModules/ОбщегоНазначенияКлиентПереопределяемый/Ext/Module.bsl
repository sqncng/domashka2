...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	ОбщегоНазначенияСППРКлиент.ПослеНачалаРаботыСистемы();
...
Процедура ПриУстановкеЗаголовкаКлиентскогоПриложения(ЗаголовокПриложения, ПриЗапуске, ПараметрыКлиента) Экспорт
	...
	// СППР
	ПредставлениеЗаголовка = ЗаголовокПриложения;
	ПредставлениеКонфигурации = ПараметрыКлиента.ПодробнаяИнформация;
	
	Проект = ПараметрыКлиента.ТекущийПроект;
	
	Если ЗначениеЗаполнено(Проект) Тогда
		ПредставлениеПроекта = НСтр("ru='Проект: '") + Проект;
	Иначе
		ПредставлениеПроекта = НСтр("ru='Проект не выбран'");
	КонецЕсли;
	
	Если ПустаяСтрока(ПараметрыКлиента.ЗаголовокПриложения) Тогда
		
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка,
			ПредставлениеКонфигурации, СокрЛП(ПредставлениеПроекта));
	Иначе
			
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
			СокрЛП(ПараметрыКлиента.ЗаголовокПриложения), СокрЛП(ПредставлениеПроекта));
